...
**Standing one up — the assisted path.** `docs/satellite-setup.md` is the full procedure written for Claude Code running on the new owner's computer: it installs nothing globally (all three CLIs run via `npx`), pops browser windows for the owner to sign into their own GitHub/Vercel/Neon/Clerk accounts, and does everything else itself. `docs/setup-guide.html` is the human-facing walkthrough to send them — it collects their sign-in email, first name, and timezone, and builds the paste-into-Claude prompt that points at the raw GitHub URL of the procedure (so both files must be on `main` for the flow to work). The manual steps below are the same procedure condensed, for a builder doing it by hand:
