...
| ~~`body_text`~~ | — | **Dropped in migration 0047 (ADR-153, "lever C").** Was a plain-text extraction of `body.text` maintained on save purely to feed the generated FTS column; it duplicated `body.text` (~40% of each item's storage). The FTS `search` column now derives its body term straight from `body ->> 'text'` (see Index plan). The stripper (`src/lib/body-text.ts`) is kept dormant as the C2 fallback. |
...
- FTS: a `GENERATED ALWAYS AS ... STORED` `tsvector` column on `items`, GIN-indexed, weighted (ADR-014): title (`A`), the body markdown via `body ->> 'text'` (`B`), then url (punctuation-split so URL words match) and `properties` string values via `jsonb_to_tsvector` (both `C`). Status/urgency/dates deliberately excluded (enums are filters, not prose). Not computed per query. **The body term reads the canonical markdown directly (ADR-153, migration 0047), not a separate `body_text` column** — that app-maintained duplicate was dropped as storage "lever C". The tradeoff: `to_tsvector` discards markdown punctuation so prose matching is unchanged, but raw markdown is no longer stripped, so chordpro chords, `ledgr://` mention URIs, and color hexes add mild index noise (concentrated in songs). The old stripper (`src/lib/body-text.ts`, `markdownToText`) is retained dormant as the C2 fallback if that noise ever needs cleaning up. *(History: through migration 0046 the tsvector fed from the app-maintained `body_text`, ADR-003.)*
