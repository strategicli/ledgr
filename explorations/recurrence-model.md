...
**Status:** ✅ **RESOLVED — ADR-076 (2026-06-17), built as T1.** All nine forks settled per the recommendations below: model (C) per-date completion log; `scheduled` as a first-class column (not a property); hand-rolled RRULE subset (no `rrule` dep, Principle 5); next-only overdue; virtual/materialized occurrence modes via one `cloneItemSubtree` primitive; fixed vs after-completion anchor; skip + COUNT/UNTIL end conditions. Kept for the record. *(Was: open brainstorm for the Native Tasks chunk T1; core, so the model landed in an ADR.)* Pairs with `tasks-todoist-vs-native.md` (the decision to go native) and `calendar-time-blocking.md` (the calendar surface, still open).
