...
**Status:** ✅ **DELIVERED — Phases 1–3 shipped 2026-06-27 (ADR-127; PRs #107, #109, #110).** Live: the **Discover related** panel (deterministic scorer + `item_relatedness` cache + nightly job + endpoint, under "Linked here"); the **Related Explorer** (`/items/[id]/explore` — score-sorted, anchor-hopping map); and **Loose Ends** (Build › Maintain `/build/loose-ends` — the engine inverted across the corpus). Remaining as opt-in extras (deferred, not core to the idea): a Loose Ends *dashboard widget* and an on-demand **AI "Find connections"** Layer 2. This exploration is kept for the record. (Originally parked Brandon, 2026-06-19, post-1.0; the sections below are the original exploration text.)
