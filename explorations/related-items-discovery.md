...
**Status:** ✅ **BUILT — Phase 1 shipped 2026-06-27 (ADR-127, PR #107).** The "Discover related" panel (deterministic scorer + `item_relatedness` cache + nightly job + endpoint, under "Linked here") is live. **Phase 2 (Related Explorer)** and **Phase 3 (Loose Ends)** remain from this doc, designed in ADR-127 and deferred. This exploration is kept for the record and as the design source for those phases. (Originally parked Brandon, 2026-06-19, post-1.0; the sections below are the original exploration text.)
