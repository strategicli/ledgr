...
**Status:** partly built — **Stage A shipped (ADR-091, 2026-06-18):** an optional scheduled start time + duration on a task (`properties.scheduledTime`), refining the scheduled calendar day, surfaced as a timed block in the published ICS feed. The rest stays **parked** (Brandon, 2026-06-15): Stage B (multi-session blocks as event items), the event-model reframe, the deterministic placer, and AI scheduling. Largely **core** (touches the data model and the calendar provider seam), so the parts that change `schema.md` or the type model land as an ADR with both-agree before they build; Stage A was `properties`-first and non-core (the column promotion is the future ADR step). Raised after looking at [Morgen](https://www.morgen.so/).
...
- **Stage A, single block on the task (cheapest). ✅ BUILT (ADR-091, 2026-06-18).** Add a scheduled start + duration to the task. The common case is one work session. Ride `properties` first (no migration, no core change) to prove the UX, then graduate to real columns (`scheduled_at timestamptz`, `scheduled_minutes int`) once it earns indexing, since "hot fields are columns" and the calendar view will query them. The column step is the core/ADR step. **(Brandon agrees with this `properties`-first → promote-to-column direction, 2026-06-15.)** *Shipped as:* `properties.scheduledTime = {start, durationMinutes}`, floating local time, refining the scheduled calendar day (not changing `scheduled_date`); the time surfaces as a timed VEVENT in the ICS feed. Column promotion still pending.
