...
**Status:** DECIDED, ready to build. This is the implementation brief for "Feature B," the second half of the 2026-06-16 item-view UX push (Feature A — inline editing — shipped in ADR-068/PR #11). Written as a **handoff to a local session** (where a browser, dev server, and `DATABASE_URL` are available — the grid needs an in-browser eyeball that a CI container can't give). It supersedes the lower-priority "item 4" in `canvas-drag-and-drop.md` and the short Feature-B notes in `next_steps.md`/`COLLAB.md` with the full, current decision set (several calls below were made in chat *after* those notes were written).
