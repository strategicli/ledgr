...
| ~~microsoft~~ | moved: `src/modules/microsoft/` (the Graph client, its health check); calendar-sync, email-capture and onedrive-export `require` it. Clerk sign-in is not part of it | none | none left |
| ~~meeting-transcripts~~ | moved: `src/modules/meeting-transcripts/` (AssemblyAI provider, transcription service, the Transcripts panel), routes gated, `transcription-poll` job gated. `lib/meetings/transcripts.ts` (the transcript item, used by the share screen), prep and promote stay core | jobs | none left |
| ~~triage~~ | moved: `src/modules/triage/` (the swipe deck); `/inbox/triage` gated; `items.inbox` stays core | none | none left |
| ~~listen~~ | moved: `src/modules/listen/` (the read-aloud bar), loaded through `module-editor.tsx`; `speechTextFor` and the per-type columns stay core. The agent module borrows its voice helpers (module to module) | none | none left |
