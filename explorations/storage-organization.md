...
> **✅ In-app org half resolved — ADR-056 (2026-06-14).** The 6.14 meeting settled the *within-Ledgr* organizing model: **relational databases (tags/categories/relations) are the primary organizing principle, not files/folders** — even "folder" is reframed as "a child of that folder" (a relation). Notes are a **module attachable to every type** rather than a folder silo, and a "project" is just relations, not a type (see `project-items.md`). The still-open part of this doc is the **export → MD-file-tree mapping** (the relation → path|wiki-link question below), which is unchanged. Read the rest with the in-app model already decided.
