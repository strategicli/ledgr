...
> **🔀 A fourth fork now has its own doc (2026-07-12).** `local-p2p-sync.md` explores "option D": the DB stays canonical but lives locally on each device and replicates peer-to-peer, with the current cloud deployment demoted to one always-on peer. It keeps rule #1 and everything-is-an-item intact, so it isn't a variant of options A-C; read it alongside this doc if local-first reopens.

> **📌 Strengthened by the 6.14 meeting (insight, not a direction change).** Running the app itself locally (Next.js on `localhost`, the whole codebase on disk) reframed local-first from "a backup of the data" to "the app *and* the data are user-owned — *this thing can't be taken*." The DB stays canonical (rule #1 holds), so this is still option A/B territory, not C. Two concrete take-aways landed: (1) **per-type, user-selectable offline caching (the "Netflix model")** — the user picks which types are cached for mobile/offline (e.g. always cache sermons before Sunday); desktop caches everything. This sharpens the Sunday-proof story and is a PWA/caching design note more than an architecture change. (2) The local run is real today (see `runbook.md` local-run procedure: `localhost`, `npm run db:migrate` after pulling schema changes), which makes the option-A "read locally, write through the API" path concrete rather than hypothetical.
