// ADR-100 verification: the editable note "date taken" (items.note_date).
...
console.log("\n# Live: createItem defaults");
{
  ...
  const note = await createItem(ownerId, { type: "note", title: "ADR-100 verify note" });
  ...
  const task = await createItem(ownerId, { type: "task", title: "ADR-100 verify task" });
...
{
  const note = await createItem(ownerId, { type: "note", title: "ADR-100 patch note" });
  created.push(note.id);
