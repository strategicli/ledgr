...
export async function GET(req: Request, ctx: { params: Promise<{ token: string }> }) {
  ...
  // Viewers have no session, so /files/ images (slide content AND the design's
  // logo/background) must be inlined as data URIs.
  const design = await inlineDesignImages(live.ownerId, deck.design);
  ...
  const html = renderPlayer({ ...deck, design, slides, mode: "follow" });
