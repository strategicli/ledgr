...
export function pairsToEditLine(pairs: ChordPair[]): EditLine {
  ...
  for (const p of pairs) {
    if (p.chord) chords.push({ at: text.length, chord: p.chord });
    text += p.text;
  }
  return { text, chords };
}
...
export function editLineToPairs(line: EditLine): ChordPair[] {
  ...
  // drop a leading empty no-chord segment the loop may produce
  if (pairs.length > 1 && pairs[0].chord === null && pairs[0].text === "") {
    pairs.shift();
  }
  return pairs;
}
