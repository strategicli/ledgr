...
- **`explorations/`**: parked fork-in-the-road ideas under consideration (not intent, not decisions). Currently: `local-first-split.md`, `project-items.md`, `block-linked-action-items.md`, `dashboard-widgets.md`, `flexible-surfaces.md` (per-view columns → custom composable pages), `template-property-defaults.md` (templates presetting people/relations), `quick-capture-at-mention.md`, `web-clipper.md`, `entity-vs-custom-type.md` (resolved — ADR-055 retired the `entity` meta-type → bespoke `person`, dropped `kind`, made the related list universal; the `relation` property kind stays deferred; kept for the record), `storage-organization.md` (how each type is organized in-app and laid out as exported MD files; relation → path/wiki-link mapping; core, needs both-agree + ADR), `scripture-passages-as-entities.md` (decided — ADR-056: a pre-built `passage` type, book→chapter→verse verse-atomic, as the relational hub; build is module-level; supersedes the doc's "entity kind" framing), `provider-seam-calendar-email.md` (extend the provider seam to calendar/email-in for Google/iCloud; core; answers PRD Q7), `confidential-tier.md` (privacy tier for pastoral content: exclude-from-MCP/export flag vs field encryption; core, decide together), and the mobile/canvas batch from the 2026-06-14 session: `mobile-swipe-navigation.md`, `notes-organization.md` (sub-pages/folders/tags), `meeting-recording.md` (upload/transcribe/minutes), `canvas-drag-and-drop.md`, `rich-export-and-theming.md`. (`confidential-tier.md`/`provider-seam-calendar-email.md`/`scripture-passages-as-entities.md` are cherry-picked from Tyler's `tyler/additions-for-review`.) (`type-and-kind-ux.md` is resolved — built in slice 33, ADR-044 — kept for the record.)
