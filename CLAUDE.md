...
- **`explorations/`**: parked fork-in-the-road ideas under consideration (not intent, not decisions). Currently: `local-first-split.md`, `project-items.md`, `block-linked-action-items.md`, `dashboard-widgets.md`, `flexible-surfaces.md` (per-view columns → custom composable pages), `template-property-defaults.md` (templates presetting people/relations), `quick-capture-at-mention.md`, `web-clipper.md`, `entity-vs-custom-type.md` (keep entities, ponder/confirm the model; revisit when the `relation` property kind lands), `storage-organization.md` (how each type is organized in-app and laid out as exported MD files; relation → path/wiki-link mapping; core, needs both-agree + ADR), `scripture-passages-as-entities.md` (a `passage` entity kind as the relational hub; core), `provider-seam-calendar-email.md` (extend the provider seam to calendar/email-in for Google/iCloud; core; answers PRD Q7), `confidential-tier.md` (privacy tier for pastoral content: exclude-from-MCP/export flag vs field encryption; core, decide together). The last three are cherry-picked from Tyler's `tyler/additions-for-review` branch (`ty-additions/`). (`type-and-kind-ux.md` is resolved — built in slice 33, ADR-044 — kept for the record.)
